...
class TC021_ConfidenceThreshold(GroupKLambdaTestBase):
    """
    POST /api/lambda/functions/<id> with a 'threshold' param must drop
    annotations whose confidence is below the threshold.
    """
    @classmethod
    def setUpTestData(cls):
        cls._create_db_users()
    ...
    def test_high_threshold_filters_low_confidence_shapes(self):
        """With threshold=0.95, only the 0.99-confidence shape should survive."""
        payload = {
            "task": self.tid,
            "frame": 0,
            "mapping": {"car": {"name": "car"}},
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        shapes = response.json()["shapes"]
        # Mock returns confidences 0.99 and 0.85; only 0.99 clears 0.95
        self.assertEqual(len(shapes), 1)

    def test_zero_threshold_keeps_all_shapes(self):
        """threshold=0.0 must not drop any annotation."""
        payload = {
            "task": self.tid,
            "frame": 0,
            "mapping": {"car": {"name": "car"}},
        ...
        with ForceLogin(self.admin, self.client):
            response = self.client.post(self.url, data=payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreater(len(response.json()["shapes"]), 1)

class TC022_PolylineAndPointsShapes(GroupKLambdaTestBase):
    """
    The _parse_anno method has branches for polyline and points types.
    Exercise both to cover those lines.
    """
    @classmethod
    def setUpTestData(cls):
        cls._create_db_users()
    ...
    def test_polyline_shape_type_is_preserved(self):
        payload = {"task": self.tid, "frame": 0, "mapping": {"car": {"name": "car"}}}
        with ForceLogin(self.admin, self.client):
            response = self.client.post(self.url, data=payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        types = {s["type"] for s in response.json()["shapes"]}
        self.assertIn("polyline", types)

    def test_points_shape_type_is_preserved(self):
        payload = {"task": self.tid, "frame": 0, "mapping": {"car": {"name": "car"}}}
        with ForceLogin(self.admin, self.client):
            response = self.client.post(self.url, data=payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        types = {s["type"] for s in response.json()["shapes"]}
        self.assertIn("points", types)

class TC023_BatchRequestWithJobParam(GroupKLambdaTestBase):
    """
    Batch requests can target a specific job (segment) instead of the whole task.
    This exercises the job-based task-lookup branch in the view.
    """
    @classmethod
    def setUpTestData(cls):
        cls._create_db_users()
    ...
    def _get_first_job_id(self):
        with ForceLogin(self.admin, self.client):
            response = self.client.get(f"/api/jobs?task_id={self.tid}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        results = response.json()["results"]
        self.assertGreater(len(results), 0)
        return results[0]["id"]

    def test_batch_request_with_job_param_is_accepted(self):
        jid = self._get_first_job_id()
        payload = {
            "function": id_function_detector,
            "job": jid,
            "cleanup": False,
...
class TC024_RequestStatusValues(GroupKLambdaTestBase):
    """
    Cover the job-status serialization paths (queued, finished, failed)
    that the retrieve/list endpoints exercise.
    """
    @classmethod
    def setUpTestData(cls):
        cls._create_db_users()
    ...
    def test_finished_request_has_status_field(self):
        payload = {
            "function": id_function_detector,
            "task": self.tid,
            "cleanup": False,
    ...
    def test_failed_request_is_retrievable(self):
        """A job that fails (simulated) must still be retrievable with a status."""
        payload = {
            "function": id_function_detector,
            "task": self.tid,
            "cleanup": False,
        ...
        with mock.patch(
            "cvat.apps.lambda_manager.views.LambdaGateway.invoke",
            side_effect=Exception("Simulated model failure"),
        ):
            with ForceLogin(self.admin, self.client):
                create = self.client.post(LAMBDA_REQUESTS_PATH, data=payload, format="json")
                # Batch job may still be created even if invoke will fail later;
                # just verify the endpoint doesn't crash on retrieve
